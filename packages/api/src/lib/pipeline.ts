...
import { renderStructuredDigestEmailHtml } from './render-digest-html';
...
import { buildStructuredDigest } from './structured-digest';
    ...
    return raw
      ...
      .map((t) => {
        ...
        const watchlist = Array.isArray(o.watchlist)
          ...
          : undefined;
        return { label, keywords, maxItems, ...(watchlist ? { watchlist } : {}) };
      })
...
export async function runDigestPipeline(env: Env, runId: string, userId: string): Promise<void> {
  ...
  try {
    ...
    if (useTopics) {
      const structured = await buildStructuredDigest(env, sources, topicConfigs, digestTz, {
        totalCap: 50,
        perTopicCap: 15,
      });
      const generatedAt = new Date().toISOString();
      const emailSubjectPrefix = 'HamHome Topic Digest';
      const summaryHtml = renderStructuredDigestEmailHtml(structured, { runId, generatedAt, title: emailSubjectPrefix });
      ...
      if (email) {
        ...
        if (!mail.ok) deliveryNotes.push(`email_failed: ${mail.error ?? 'unknown'}`);
      }
      ...
      if (mqttSuffix && env.MQTT_NOTIFY_URL) {
        ...
        const first = structured.topics[0]?.coreJudgment ?? 'Digest generated';
        const payload = JSON.stringify({ runId, userId, text: first.slice(0, 1200), at: generatedAt });
        const mq = await publishDigestMqtt(env, topic, payload);
        if (!mq.ok) deliveryNotes.push(`mqtt_failed: ${mq.error ?? 'unknown'}`);
      }
      const combinedError = [...structured.sourceErrors, ...deliveryNotes].filter(Boolean).join(' | ') || null;
      await db.update(schema.digestRuns)
    ...
    for (const src of sources) {
      try {
        const items = await fetchSourceContent(src, env);
        for (const it of items.slice(0, rssCap)) {
          agg.push({
            ...
            ...(it.skipDateFilter ? { skipDateFilter: true } : {}),
          });
        }
      } catch (e) {
...
export async function runDigestPipelineForSubscription(
  ...
  try {
    ...
    const structured = await buildStructuredDigest(env, sources, topicConfigs.map((t) => ({
      ...t,
      maxItems: mode === 'test' ? Math.min(5, t.maxItems ?? perTopicCap, perTopicCap) : Math.min(t.maxItems ?? perTopicCap, perTopicCap),
    })), subTz, {
      totalCap: mode === 'test' ? Math.min(totalCap, 8) : totalCap,
      perTopicCap: mode === 'test' ? Math.min(perTopicCap, 5) : perTopicCap,
    });

    const generatedAt = new Date().toISOString();
    const prefix = mode === 'test' ? '[TEST] HamHome 订阅日报' : 'HamHome 订阅日报';
    const summaryHtml = renderStructuredDigestEmailHtml(structured, { runId, generatedAt, title: prefix });
